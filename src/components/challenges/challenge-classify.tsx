...
interface ChallengeClassifyProps {
  challengeId: string;
  categories: string[];
  items: ClassifyItem[];
  closingNote?: string;
...
export default function ChallengeClassify({
  challengeId,
  categories,
  items,
  closingNote,
}: ChallengeClassifyProps) {
  ...
  function handleAnswer(itemIndex: number, category: string) {
    setAnswers((prev) => {
      ...
      if (Object.keys(next).length === items.length) {
        const correct = items.filter((it, i) => next[i] === it.correctCategory).length;
        recordResult(challengeId, correct, items.length);
      }
      return next;
    });
  }
  ...
  return (
    <div className="space-y-4">
      {items.map((item, itemIndex) => {
        ...
        return (
          <div key={itemIndex} className="rounded-lg border border-black/10 p-3 sm:p-4">
      ...
      {allAnswered && closingNote && (
        <p className="rounded-lg border border-brand-purple/30 bg-brand-purple/5 p-3 text-xs text-foreground">
          {closingNote}
        </p>
      )}
    </div>
